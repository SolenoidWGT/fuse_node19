...
// 添加
static int reconnect(struct dhmp_transport* rdma_trans)
{
	rdma_trans->nvm_used_size = 0;
	rdma_trans->dram_used_size = 0;
	int i = rdma_trans->node_id;
	dhmp_transport_connect(rdma_trans, client->config.net_infos[i].addr, client->config.net_infos[i].port);
	return 0;
}

// 添加
static int recovery_thread(void* _rdma_trans)
{
	pthread_detach(pthread_self());
	struct dhmp_transport* rdma_trans = (struct dhmp_transport*)_rdma_trans;
	while(rdma_trans->trans_state != DHMP_TRANSPORT_STATE_CONNECTED){
        reconnect(rdma_trans);
        sleep(1);
    }
	client->recovery(rdma_trans->node_id);
...
{
	// 此事件表示远端发生了错误，并返回一个无效的请求，。
	// 可能是请求队列不支持该操作，或者缓冲区空间不足以接收请求。
...
static void dhmp_comp_channel_handler(int fd, void* data)
{
	...
	//int i=0;
	// 这个地方应该是一个bug
	// 服务器断开连接后，日志会输出大概8192个i，然后出现一个cm event [RDMA_CM_EVENT_REJECTED],status:8信息，然后重新计数
...
static int on_cm_disconnected(struct rdma_cm_event* event, struct dhmp_transport* rdma_trans)
{
	...
	if(client!=NULL)
	{
		if(client->prepare_destroy){
			dhmp_destroy_source(rdma_trans);
		}else{
			if(!client->in_recovery){
				client->in_recovery = true;
				pthread_create(&client->recovery_thread, NULL, recovery_thread, (void*)rdma_trans);
			}
		}
	}
...
static int dhmp_handle_ec_event(struct rdma_cm_event* event)
{
	...
	switch(event->event)
	{
		...
		case RDMA_CM_EVENT_CONNECT_ERROR:
			...
			break;
		// RDMA_CM_EVENT_REJECTED指示连接请求或响应被远程端点拒绝。
		case RDMA_CM_EVENT_REJECTED: // 添加这个case
			break;
		default:
								...
								off_t offset)
{
	...
	DEBUG_LOG("before read_mr[%d]", rdma_trans->node_id);
	...
	*/
	...
	DEBUG_LOG("after read_mr[%d]", rdma_trans->node_id);
