...
#define TOTOL_SIZE ((uint64_t)1024*1024*1024*10)     // 磁盘总空间,2G
...
//         DEBUG_END();
	...
	return;
#endif	
	uintptr_t globe_addr,local_addr;
	{ 
		// //int dhmp_read(void *dhmp_addr, void * local_buf, size_t length);
		// dhmp_read(tbank + offset, buf, size);
		// return;

	#ifdef octopus
		model_1_octopus(tbank + offset, size, buf, 0);
	#endif
	#ifdef L5
		model_5_L5(size, buf, tbank + offset, 0);
	#endif
	#ifdef FaRM
		globe_addr = (uintptr_t) (tbank + access_off);
		local_addr = (uintptr_t) (dhmp_local_buf + access_off);
		model_FaRM(globe_addr, ACCESS_SIZE, local_addr, 0 , 1);
		check_request(1);
	#endif
	#ifdef RFP
		model_4_RFP(ACCESS_SIZE, dhmp_local_buf + access_off, tbank + access_off, 0); //1 for write
	#endif
	#ifdef scaleRPC
		globe_addr = (uintptr_t) (tbank + offset);
		local_addr = (uintptr_t) (buf);
		model_7_scalable(&globe_addr, size, &local_addr, 0 , 1);
	#endif
	#ifdef DaRPC
		globe_addr = (uintptr_t) (tbank + offset);
		local_addr = (uintptr_t) (buf);
		model_3_DaRPC(&globe_addr, size, &local_addr, 0, 1); //用
		check_request(3);
	#endif
	}
...
return ;
	...
	{ 
	#ifdef octopus
		model_1_octopus(tbank + offset, size, buf, 1);
	#endif
	#ifdef L5
		model_5_L5(size, buf, tbank + offset, 1); 
	#endif
	#ifdef FaRM
		globe_addr = (uintptr_t) (tbank + access_off);
		local_addr = (uintptr_t) (dhmp_local_buf + access_off);
		model_FaRM(globe_addr, ACCESS_SIZE, local_addr, 1 , 1);
		check_request(1);
	#endif
	#ifdef RFP
		model_4_RFP(ACCESS_SIZE, dhmp_local_buf + access_off, tbank + access_off, 1); //1 for write
	#endif
	#ifdef scaleRPC
		globe_addr = (uintptr_t) (tbank + offset);
		local_addr = (uintptr_t) (buf);
		model_7_scalable(&globe_addr, size, &local_addr, 1 , 1);
	#endif
	#ifdef DaRPC
		globe_addr = (uintptr_t)(tbank + offset);
		local_addr = (uintptr_t) (buf);
		model_3_DaRPC(&globe_addr, size, &local_addr, 1, 1); //用
		check_request(3);
	#endif
	}
}
...
//         DEBUG_END();
	...
	while(un_write_size > 0)
	{
		if(un_write_size >= (CHUNK_SIZE - write_offset)){
			...
			dhmp_fs_write_to_bank(cnt->chunk_index, (char*)buf + write_size, (CHUNK_SIZE - write_offset), write_offset);
		...
		{
			// 将剩下的未写入数据都写进去
			dhmp_fs_write_to_bank(cnt->chunk_index, (char*)buf + write_size , un_write_size, write_offset);
