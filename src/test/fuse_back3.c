#include "../include/fuse_dhmp.h"
#include "../include/fuse_logs.h"
...
void dhmp_fs_read_from_bank( int chunk_index, char * buf, size_t size, off_t chunk_offset)
{
	...
	//FUSE_INFO_LOG("chunk_index is %d, read size is %d, chunk_offset is %d", chunk_index, size, chunk_offset);
	...
	while(read_server){
		void * tbank = read_server->bank[chunk_index / (CHUNK_NUM/BANK_NUM)];
		int re = dhmp_read(tbank, buf, size, offset);
		if(re == -1){
			FUSE_ERROR_LOG("Server has down! It is starting to retry");
			read_server->alive = 0;
			read_server = find_alive_server();
		}
		else{
			//FUSE_INFO_LOG("dhmp_read sucess!");
			return;
		}
	}
...
void chose_alive_server(int back_num, int aliveNums, server_node ** serverBackup){
	int i, j;
	for(i=0; i<back_num; i++){
		...
		while(rid == -1){
			...
			rid = rand() % aliveNums;		// 一致性hash


			if(!fuse_sList[rid]->alive){
				rid = -1;
				continue;
			}
			for(j=0; j<back_num; j++){
				if(serverBackup[j] != NULL && rid == serverBackup[j]->server_id){
					rid = -1;
					break;
				}else if(serverBackup[j] == NULL){
					serverBackup[j] = fuse_sList[rid];
					break;
				}
			}
		}
...
void dhmp_fs_write_to_bank(int chunk_index, char * buf, size_t size, off_t chunk_offset, context *cnt)
{
	//FUSE_INFO_LOG("chunk_index is %d, read size is %d, chunk_offset is %d", chunk_index, size, chunk_offset);
	...
	// 开辟线程写服务器
	int rid = -1, j;
	int aliveNums = dhmp_get_trans_count();
	int back_num = BACKUPNUMS;
	bool bad_backup = false;
	if(aliveNums < BACKUPNUMS){
		FUSE_ERROR_LOG("aliveNums < backnums, Cant's keep %d backups!", BACKUPNUMS);
		back_num = aliveNums;
		bad_backup = true;
	}
	pthread_t threads[back_num]; 
	writeThreadArgs ** all_args = (writeThreadArgs **) malloc(back_num * sizeof(writeThreadArgs *));
	server_node ** serverBackup = (server_node **)malloc(back_num * sizeof(server_node *));
	for(i=0; i<back_num;i++){
		serverBackup[i]= NULL;
	}
	...
	for(i=0; i<back_num; i++){
		...
		// need modify
		chose_alive_server(back_num, aliveNums, serverBackup);

		// cnt->backups[i] = rid;
		

		void * tbank = (fuse_sList[rid]->bank[chunk_index / (CHUNK_NUM/BANK_NUM)]);  
	...
	for(i=0; i<back_num; i++){
		...
		if(all_args[i]->re == -1){
			// 我们是否将返回-1认为是断线，还是仅仅认为是丢包？
			FUSE_ERROR_LOG("Server %d is disconnected, Can't write!", serverBackup[i]);
			//cnt->backups[i] = -1;
			// if therer are others alive servers, we will retry
			if(!bad_backup){
				FUSE_ERROR_LOG("aliveNums < backnums, Cant rewrite, abandon.");
			}else{
				// retry
			...
			//FUSE_INFO_LOG("Sucess write chunk %d to server %d", chunk_index, serverBackup[i]);
		}
		free(all_args[i]);
	}
	return;
...
int dhmpMknod(const char * path)
{
	...
	if(father == NULL) {FUSE_ERROR_LOG("EROrr path to mknod\n");return -1;}
	{
...
int dhmpGetAttr(const char *path,struct attr *attr)
{
	...
	//FUSE_INFO_LOG("get atter size = %d", attr->size );
	return 0;
}
...
static int dhmp_fs_read(const char *path, char *buf, size_t size, off_t offset,
		    struct fuse_file_info *fi)
{
	//FUSE_INFO_LOG("Read Path: %s, Read size: %d, Read offset %d", path, size, offset);
	...
	while(i > 0)
	{
		if(cnt == NULL || cnt -> size  != CHUNK_SIZE){ FUSE_ERROR_LOG("dhmp_fs_read_error");return 0;}
	...
	{
		if(cnt == NULL) {FUSE_ERROR_LOG("dhmp_fs_read_eror2");break;}
	...
	free(bbuf);
	//FUSE_INFO_LOG("After Read size: %d, Read offset %d, Read size is %d", size, offset, read_size);
	return read_size;
}

context * malloc_context(){
	context * cnt = (context *)malloc(sizeof(struct context));
	cnt->size = 0;
	cnt->chunk_index = getFreeChunk();
	if(cnt->chunk_index == -1){
		free(cnt);
		return NULL;
	}
	cnt->next = NULL;
#ifdef DHMP_ON
	// cnt->backups = (int*) malloc(BACKUPNUMS * sizeof(int));
	// // 设置每一个chunk的副本位置
	// int j;
	// for(j=0; j<BACKUPNUMS; j++)
	// 	cnt->backups[j] = -1;

#endif
	//FUSE_INFO_LOG("getFreeChunk %d",cnt->chunk_index);
	return cnt;
}
...
{
	//FUSE_INFO_LOG("write Path: %s, write size: %d, write file offset is %d", path, size, offset);
	int res;
	char filename[FILE_NAME_LEN];
	int len = strlen(path);
	strcpy(filename,path);
	...
	if(head->context == NULL)
		head->context = malloc_context();
	struct context * cnt = head->context;

	off_t write_offset = offset;
	int i = write_offset / CHUNK_SIZE;	// 需要跳过多少个chunk
	write_offset = write_offset % CHUNK_SIZE;
	//FUSE_INFO_LOG("skip chunk nums are %d, write chunk offset is %d", i, write_offset);
	while(i > 0)
	{
		if(cnt == NULL)
		{
			FUSE_ERROR_LOG("In Write cnt == NULL!, return 0");
			return 0;
		}
		...
		{
			cnt->next = malloc_context();	// 如果write_offset恰好为0，且已经遍历到chunk list的末尾，则需要重新开一个新的chunk
		}
		cnt = cnt->next;
	}
	...
	while(un_write_size > 0)
	{
		if(un_write_size >= (CHUNK_SIZE - write_offset)){
			...
			dhmp_fs_write_to_bank(cnt->chunk_index, buf + write_size, (CHUNK_SIZE - write_offset), write_offset, cnt);
			...
			{	
				cnt->next = malloc_context();
			}
		}
		else
		{
			// 将剩下的未写入数据都写进去
			dhmp_fs_write_to_bank(cnt->chunk_index, buf + write_size , un_write_size, write_offset, cnt);
